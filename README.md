...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/Specialization-Realtime%20Services%20%7C%20Search%20%7C%20AI/ML-green?style=for-the-badge" alt="Specialization">
</p>
...
### 🔍 [Dorandoran: YT2 Search System](https://github.com/DoHyunDaniel/yt2_search_project)
> **여행/YouTube 검색 서비스 Dorandoran의 검색 엔진 & API 서버**  
> `Python` `FastAPI` `PostgreSQL` `OpenSearch` `React` `AI/ML`

**핵심 기능:**
- 🔍 **7가지 검색 알고리즘**  
  (기본, TF-IDF, 가중치, BM25, 하이브리드, 의미 기반, 감정 분석 기반 검색)
- 📊 **AI 기반 통계 분석**  
  (인기 비디오/채널 분석, 카테고리/기간별 트렌드 분석)
- 🎯 **AI 추천 시스템**  
  (콘텐츠 기반, 인기도 기반, 트렌드 기반 추천)
- ⚡ **실시간 디바운싱 검색** (800ms) & 검색 결과 캐싱
- 📡 **YouTube API 통합** & 자동 크롤링/데이터 적재 파이프라인
- 🎨 **반응형 UI/UX** & Material Design 기반 인터페이스

**기술 스택:**  
`Python 3.11` `FastAPI` `PostgreSQL` `OpenSearch` `Redis` `React 18` `Docker` `scikit-learn`

---

### 💰 [MoneyBuddy](https://github.com/MoneyBuddyTeam/BE)
> **전문가 상담 & 소비습관 개선 플랫폼 (실시간 채팅 + 포인트 결제)**  
> `Java` `Spring Boot` `Redis` `WebSocket` `OAuth2`

**핵심 기능:**
- 🔐 **JWT + OAuth2.0** (Google/Naver/Kakao/Email) 기반 인증 시스템
- 🔁 **Refresh Token + 자동 재발급 구조** 및 Axios 인터셉터 연동
- 💬 **Redis Pub/Sub & WebSocket (STOMP)** 기반 1:1 실시간 채팅
- 💳 **포인트 충전/차감 기반 결제 흐름** (PG 대체 구조)
- 📸 **AWS S3 Presigned URL 이미지 업로드**
- 🧑‍💼 **전문가/사용자/관리자 역할 분리 & 관리자 기능(공지/FAQ/CS 설계)**

**기술 스택:**  
`Java 17` `Spring Boot` `JPA` `Spring Security` `MySQL` `Redis` `Docker` `AWS EC2/S3`

---

### 💬 [MoneyTalk](https://github.com/DoHyunDaniel/moneytalk)
> **중고거래 & 가계부 챗봇 플랫폼 (Monolithic v1)**  
> `Java` `Spring Boot` `WebSocket` `OAuth2`

**핵심 기능:**
- 🔐 **JWT + OAuth2.0** 기반 로그인/회원관리
- 💬 **Redis Pub/Sub & WebSocket** 실시간 채팅 (구매자–판매자 간 1:1 채팅)
- 📸 **AWS S3 이미지 업로드 & 리뷰 이미지 관리**
- ⭐ **리뷰 & 평점 시스템**, 위시리스트, 거래 내역 관리
- 📒 **가계부 & 예산 관리 + AI 챗봇**을 활용한 소비 패턴 분석(프로토타입)
...
- ☁️ **MoneyTalk v2 (WIP)** – MoneyTalk를 MSA 구조로 재구성한 프로젝트  
  `Spring Cloud Gateway` `Auth/User 서비스 분리` `Docker Compose` `GitHub Actions CI/CD` 중심으로 인프라와 인증 구조를 재설계 중입니다.
...
![FastAPI](https://img.shields.io/badge/FastAPI-0.104%2B-green?style=flat-square&logo=fastapi&logoColor=white)
...
![GitHub Actions](https://img.shields.io/badge/GitHub%20Actions-CI%2FCD-blue?style=flat-square&logo=github-actions&logoColor=white)

### AI/ML & Security
![scikit-learn](https://img.shields.io/badge/scikit--learn-ML-orange?style=flat-square&logo=scikitlearn&logoColor=white)
